...
- `AERIAL_E2E_AUTH_SMOKE=1 ... AERIAL_E2E_EXPECT_RASTER=1 npm run test:e2e -- authenticated-ops.spec.ts` - pass, including support-answer generation followed by `/admin/copilot` audit-panel verification for `support-assistant`, `succeeded`, and sentence/drop counts.
