...
- `npm run build` - pass.
- `AERIAL_E2E_BASE_URL=https://aerial-intel-platform-nu652wicf-natford.vercel.app AERIAL_E2E_SKIP_SERVER=1 npm run test:e2e` - pass, 2 public showcase tests and 1 skipped authenticated smoke.
- `AERIAL_E2E_AUTH_SMOKE=1 ... AERIAL_E2E_EXPECT_RASTER=1 npm run test:e2e -- authenticated-ops.spec.ts` against `https://aerial-intel-platform-nu652wicf-natford.vercel.app` - pass.
...
```text
https://aerial-intel-platform-nu652wicf-natford.vercel.app
```

Vercel deployment:

```text
dpl_CLdwCa8cxBEkV5BsWLo6hH5LaiQX
```
...
- Copilot processing-QA citations: `/jobs/11111111-1111-4111-8111-111111111111` returned visible per-sentence citations with 9 `[fact:*]` tokens, `4/4 sentences kept`, and spend `$0.061` on `anthropic/claude-opus-4.7`.
...
For this Preview, `AERIAL_TITILER_URL=https://titiler.xyz` was passed as a deployment runtime/build env. The Toledo COG artifact page for `6c413396-7475-4010-a1fe-b90cbc22977a` rendered the raster viewer, loaded at least one `200` TiTiler tile, and did not render the "Viewer not configured" fallback.
...
2. Add `AERIAL_TITILER_URL` to the Vercel Preview environment permanently; the current proof deployment injected it with `vercel deploy -e/-b`.
3. Promote the authenticated smoke from a live-dev fixture to a dedicated test Supabase project.
4. Keep `https://titiler.xyz` only as temporary Preview evidence until the controlled raster plane exists.
