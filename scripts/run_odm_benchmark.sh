...
Environment:
  ...
  ODM_EXTRA_ARGS    Extra ODM CLI arguments appended in deterministic mode (whitespace-split; no shell quoting).
  ODM_ARGS          Full ODM argument override (advanced/legacy; bypasses deterministic defaults; whitespace-split).
...
if [[ ! -d "$DATASET_ROOT" ]]; then
  preflight_fail "dataset_root does not exist: $DATASET_ROOT"
...
if [[ ! -d "$IMAGES_PATH" ]]; then
  preflight_fail "expected images folder at: $IMAGES_PATH"
...
if [[ "$IMAGE_COUNT" -eq 0 ]]; then
  preflight_fail "no supported image files found in $IMAGES_PATH"
...
if [[ "$FREE_GB" -lt "$MIN_FREE_GB" ]]; then
  preflight_fail "only ${FREE_GB}GB free on dataset volume; require at least ${MIN_FREE_GB}GB"
...
# Mount the dataset's parent dir at /datasets so ODM can read images and write sibling
# output folders (odm_orthophoto/, odm_dem/, etc.) inside ODM_PROJECT_NAME. Anything in
# $DATASET_PARENT is visible to the container; keep benchmark inputs under a dedicated
# directory and do not point DATASET_ROOT at $HOME or similarly broad paths.
...
KEY_OUTPUTS_PRESENT=0
{
  ...
  for rel_path in "${EXPECTED_OUTPUTS[@]}"; do
    ...
    if [[ -s "$abs_path" ]]; then
      file_status="present"
      bytes="$(file_size "$abs_path")"
      KEY_OUTPUTS_PRESENT="$((KEY_OUTPUTS_PRESENT + 1))"
    elif [[ -e "$abs_path" ]]; then
      file_status="empty"
      bytes="$(file_size "$abs_path" 2>/dev/null || echo 0)"
    else
      file_status="missing"
      bytes=0
    fi
